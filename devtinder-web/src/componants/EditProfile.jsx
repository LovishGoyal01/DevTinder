...
const EditProfile = ({ user }) => {
  ...
  const parsedSkills = skillsInput.split(",").map(s => s.trim().slice(0, MAX_SKILL_LENGTH)).filter(Boolean);
  ...
  const uploadToCloudinary = async (file) => {
    ...
    try {
      const { data } = await axios.post("https://api.cloudinary.com/v1_1/dkfpmhm1h/image/upload", formData);
  ...
  const handleAIImprove = async () => {
   try {
    ...
    const { data } = await axios.get( Base_URL + "/profile/gptAbout", { withCredentials: true }
     );
    ...
    <div className="flex flex-col justify-center items-center h-64 gap-4">
      ...
      <div className="flex gap-4">
        <button className="btn bg-green-500 text-white"
          onClick={() => { setAbout(aiAbout); setShowAIPreview(false); }}
        >
          Accept
        </button>

        <button className="btn bg-gray-300"  onClick={() => setShowAIPreview(false)}>
          Reject
        </button>
      </div>
    </div>
         ...
         <div className="card-body p-4">
          ...
          <div className="mt-2">
            <label className="text-sm font-medium text-gray-600">Profile Photo</label>
             <button type="button" className="ml-2 btn btn-sm bg-white border-2 border-gray-300 rounded-lg text-gray-700 hover:bg-gray-100 hover:border-blue-500 transition-all duration-150"
        ...
        {/* Gender & Age */}
           ...
           <div className="mt-2">
             <div className="flex justify-between">
                ...
                <button type="button" onClick={handleAIImprove} className="text-xs font-medium text-pink-500 hover:text-sm hover:font-semibold ">
                 ✨ Improve with AI
                </button>
             </div>
           ...
           {/* Save */}
           <button disabled={parsedSkills.length === 0 ||!photoURL || photoURL === "Uploading..." || about.length < 75 } className="btn bg-pink-500 disabled:bg-pink-300 hover:bg-pink-600 text-white mt-2" onClick={handleEdit}>
             Save Changes
           </button>
         </div>
       </div>
